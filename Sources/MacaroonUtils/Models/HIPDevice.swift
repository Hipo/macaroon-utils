...
open class HIPDevice: Printable {
    ...
    public init() {
        ...
        model =
            modelData?.withUnsafeBufferPointer {
                dataPointer -> String? in
        ...
        if let systemIdentiifer = UIDevice.current.identifierForVendor?.uuidString {
            ...
            if let cacheIdentifier = userDefaults.string(forKey: cacheIdentifierKey) {
